...
namespace prep.collections
{
    public class MovieLibrary
    {
        ...
        {
            return
                movies.all_matching(x => x.date_published.Year > year);
        }

        public IEnumerable<Movie> all_movies_published_between_years(int startingYear, int endingYear)
        {
            return
                movies.all_matching(x => startingYear <= x.date_published.Year && x.date_published.Year <= endingYear);
        }

        public IEnumerable<Movie> all_kid_movies()
        {
            return
                movies.all_matching(x => x.genre == Genre.kids);
        }

        public IEnumerable<Movie> all_action_movies()
        {
            return
                movies.all_matching(x => x.genre == Genre.action);
        }
